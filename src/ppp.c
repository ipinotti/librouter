...
void ppp_pppd_arglist (char **arglist, ppp_config *cfg, int server)
{
	...
	if (cfg->sync_nasync) {
		...
		if (server) {
			...
			if (cfg->chat_script[0]) {
				...
				snprintf (filename, 99, PPP_CHAT_FILE,
				                cfg->chat_script);
				filename[99] = 0;
				MKARG("connect");
#if 1
	...
	if (server) {
		if (cfg->server_ip_addr[0] || cfg->server_ip_peer_addr[0]) {
			char buf[40];
			snprintf (buf, 39, "%s:%s", cfg->server_ip_addr,
	...
	if (cfg->default_route) {
		MKARG("defaultroute");
	} /* only work if system has not a default route allready */
	else {
		MKARG("nodefaultroute");
	}

	if (cfg->auth_user[0] && ((cfg->server_flags & SERVER_FLAGS_PAP)
	...
	MKARG("noauth"); /* Will not ask for the peer to authenticate itself */
	...
	if (cfg->ipx_enabled) {
		...
		snprintf (ipx_network, 10, "%08lX",
		                (unsigned long int) cfg->ipx_network);
		ipx_network[8] = 0;
		sprintf (ipx_node, "%02x%02x%02x%02x%02x%02x",
		                cfg->ipx_node[0], cfg->ipx_node[1],
		                cfg->ipx_node[2], cfg->ipx_node[3],
		                cfg->ipx_node[4], cfg->ipx_node[5]);
		MKARG("ipx");
		MKARG("ipx-network");
		MKARG(ipx_network);
		MKARG("ipx-node");
