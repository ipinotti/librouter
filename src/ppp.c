 ...
 */
int libconfig_ppp_verif_param_backupd(char *field, char *value, char * intf_return){
...
void libconfig_ppp_pppd_arglist(char **arglist, ppp_config *cfg, int server)
{
	...
	if (cfg->ipx_enabled) {
		...
		sprintf(ipx_node, "%02x%02x%02x%02x%02x%02x", cfg->ipx_node[0],
		                cfg->ipx_node[1], cfg->ipx_node[2],
		                cfg->ipx_node[3], cfg->ipx_node[4],
		                cfg->ipx_node[5]);
