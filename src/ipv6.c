...
void librouter_ipv6_ethernet_set_addr(char *ifname, char *addr, char *mask) /* main ethernet interface */
{
...
{
	librouter_ipv6_addr_flush(ifname); /* Clear all interface addresses */
}
...
//		conf->mtu = info->link[i].mtu;
		...
		for (j = 0; j < MAX_NUM_IPS; j++, ipaddr++) {
			...
			if (!strcmp(interface, ipaddr->ifname)) {
				inet_pton(AF_INET6, ipaddr->local.s6_addr, main_ip->ipv6addr);
				librouter_ipv6_bitlen2mask(ipaddr->bitlen, main_ip->ipv6mask);
...
//					inet_pton(AF_INET6, ipaddr->remote.s6_addr, main_ip->ippeer);
		...
		for (j = 0; j < MAX_NUM_IPS; j++, ipaddr++) {
			...
			if (strcmp(name, ipaddr->ifname) == 0) {
				inet_pton(AF_INET6, ipaddr->local.s6_addr, sec_ip->ipv6addr);
				librouter_ipv6_bitlen2mask(ipaddr->bitlen, sec_ip->ipv6mask);
				sec_ip++;
			}
		}
