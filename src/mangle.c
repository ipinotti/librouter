		...
		if (strncmp(buf, "Chain ", 6) == 0) {
			...
			if ((strlen(buf)) && ((xmangle == NULL) || (strcmp(xmangle, mangle) == 0))) {
				...
				if ((icmp_type = strstr(buf, "icmp type "))) {
					...
					if ((icmp_type_code = strstr(buf, "code ")))
						icmp_type_code += 5;
				}
				...
				if ((strcmp (type, "DSCP") == 0) || (strcmp (type, "MARK") == 0)) {

					/* filter CHAINs */
					if (strcmp(mangle, "INPUT") != 0 &&
						strcmp(mangle, "PREROUTING") != 0 &&
						strcmp(mangle, "OUTPUT") != 0 &&
						strcmp(mangle, "POSTROUTING") != 0) {

						if ((!manglep) && (!conf_format)) {
							fprintf(F, "MARK rule %s\n", mangle);
						}
						...
						print_mangle(type, dscp, mark, prot, source,
						                dest, sports, dports, mangle,
						                F, conf_format, atoi(mcount),
						                flags, tos, dscp_match, state,
