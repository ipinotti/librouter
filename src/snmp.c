...
{
	return find_string_in_file_nl(FILE_SNMPD_CONF, "syscontact", buffer, max_len);
}

int snmp_get_location(char *buffer, int max_len)
{
	return find_string_in_file_nl(FILE_SNMPD_CONF, "syslocation", buffer, max_len);
}
...
{
	return replace_string_in_file_nl(FILE_SNMPD_CONF, "syslocation", location);
}
...
int snmp_stop(void)
{
	if (is_snmp_running()) {
		...
		if( (ret = kill_daemon(SNMP_DAEMON)) == 0 ) {
			for( timeout=time(NULL)+10; (get_pid(SNMP_DAEMON) == pid) && (time(NULL) < timeout); )
				usleep(100000);
		}
		send_rmond_signal(SIGUSR1);
		return ret;
...
int snmp_set_community(const char *community_name, int add_del, int ro)
{
	...
	while (!feof (F))
	{
		...
		if (strncmp(buf, ro ? "rocommunity" : "rwcommunity", 11) == 0)
		{
			...
			{
				if (!add_del) continue; /* delete community */
...
int snmp_dump_communities(FILE *out)
{
	...
	while (!feof (F))
	{
		...
		buf[1023] = 0;
		striplf (buf);
		if ((strncmp (buf, "rocommunity", 11) == 0)||
		    (strncmp (buf, "rwcommunity", 11) == 0))
		{
...
int do_rmon_event_log(int index, char *description)
{
	...
	if( test_file_write_size(FILE_RMON_LOG_EVENTS, FILE_RMON_LOG_EVENTS_MAX_SIZE, buf, strlen(buf)) < 0 ) {
		free(buf);
		return -1;
	}
	free(buf);
...
int do_rmon_add_event(int num, int log, char *community, int status, char *descr, char *owner)
{
...
int do_rmon_event_show(char *index)
{
	...
	if( get_access_rmon_config(&shm_rmon_p) ) {
		...
		for(i=0; i < NUM_EVENTS; i++) {
			if( idx ) {
				if( shm_rmon_p->events[i].index == idx ) {
					show_event_info(shm_rmon_p, i, shm_rmon_p->events[i].index);
					ret = 0;
					break;
				}
			}
...
void show_alarm_info(struct rmon_config *shm_rmon_p, int i, int idx)
{
	...
	if( strlen(shm_rmon_p->alarms[i].str_oid) && shm_rmon_p->alarms[i].interval )
		printf("  Monitors %s every %d second(s)\n", convert_oid_to_object_name(shm_rmon_p->alarms[i].str_oid, buf, 50) ? buf : shm_rmon_p->alarms[i].str_oid, shm_rmon_p->alarms[i].interval);
...
int do_rmon_alarm_show(char *index)
{
	...
	if( get_access_rmon_config(&shm_rmon_p) ) {
		...
		for(i=0; i < NUM_ALARMS; i++) {
			if( idx ) {
				if( shm_rmon_p->alarms[i].index == idx ) {
					show_alarm_info(shm_rmon_p, i, shm_rmon_p->alarms[i].index);
					ret = 0;
					break;
				}
			}
...
int add_pdu_data_entry(struct trap_data_obj **data_p, char *oid_str, int type, char *value)
{
	int idx;
...
int sendtrap(char *snmp_trap_version, char *community_rcv, char *trap_obj_oid, struct trap_data_obj *trap_data)
{
	FILE *f;
	netsnmp_pdu *pdu;
	size_t name_length;
			...
			switch( session.version ) {
				...
				case SNMP_VERSION_3:
					...
					for(k=0, errors=0; trap_data[k].oid; k++) {
						...
						if( snmp_translate_oid(trap_data[k].oid, name, &name_length) ) {
							type = '\0';
...
static int adjust_snmp_fdb(unsigned int add, char *line, int rw)
{
	...
	case 1:
		...
		/* Ajusta arquivo FILE_SNMPD_STORE_CONF */
		if( copy_file_to_file(SNMP_USERKEY_FILE, FILE_SNMPD_STORE_CONF) < 0 )
			break;
		...
		if( (n = parse_args_din(line, &argl)) >= 2 ) {
			if( (strcmp(argl[0], "createUser") == 0) && (strchr(line, '\n') != NULL) ) {
				if( add_user_snmpd_conf(argl[1], rw, ((n == 3) ? 
					"noauth" : ((n == 5) ? "auth" : "priv"))) >= 0 )
					found = 1;
			}
		}
		free_args_din(&argl);
...
{
	int ret = 0, is_running = is_snmp_running() ? 1 : 0;
...
int add_snmp_user(char *user, int rw, char *authpriv, char *authproto, char *privproto, char *authpasswd, char *privpasswd)
{
	FILE *f;
	arg_list argl = NULL;
	char auth[8], buf[256];
	int len, ret = 0, found = 0, is_running = is_snmp_running() ? 1 : 0;
	...
	len = 6 + strlen("createUser ") + strlen(user) + ((authproto != NULL) ? (7 + strlen(authpasswd)) : 0) + ((privpasswd != NULL) ? (5 + strlen(privpasswd)) : 0);
	if( len > 256 ) {
		ret = -1;
		goto error;
	}
	...
	if( (f = fopen(SNMP_USERKEY_FILE, "r")) != NULL ) {
		while( (feof(f) == 0) && (found == 0) ) {
			if( fgets(buf, 255, f) != NULL ) {
				buf[255] = 0;
				if( parse_args_din(buf, &argl) >= 2 ) {
					if( (strcmp(argl[0], "createUser") == 0) && (strcmp(argl[1], user) == 0) )
						found = 1;
				}
				free_args_din(&argl);
			}
			...
			while( feof(f) == 0 ) {
				if( fgets(buf, 255, f) != NULL ) {
					...
					if( (n = parse_args_din(buf, &argl)) >= 2 ) {
						if( (strcmp(argl[0], "createUser") == 0) && (strchr(buf, '\n') != NULL) ) {
							/* Adiciona entrada de usuario dentro do arquivo "/etc/snmpd.conf" */
							add_user_snmpd_conf(argl[1], ((strcmp(argl[n-1], "#rw") == 0) ? 1 : 0), ((n == 3) ? "noauth" : ((n == 5) ? "auth" : "priv")));
						}
					}
					free_args_din(&argl);
				}
