...
{
	return do_chflags(dev, 0, IFF_UP);	
}
...
int dev_get_link(char *dev)
{
	...
	if (dev_get_flags(dev, &flags) < 0) return (-1);
	return (flags&IFF_UP);
}
...
int arp_del(char *host) /* <ipaddress> */
{
	struct arpreq req;
	struct sockaddr sa;
...
int arp_add(char *host, char *mac) /* <ipaddress> <mac> */
{
	struct arpreq req;
	struct sockaddr sa;
	struct sockaddr_in *sin;
...
int get_interface_buffers_use(char *dev, char *tx, char *rx, unsigned int len)
{
	char *p;
	int sock;
	struct ifreq ifr;
...
int dev_set_rxring(char *dev, int size)
{
	int s;
	struct ifreq ifr;
...
int dev_set_txring(char *dev, int size)
{
	int s;
	struct ifreq ifr;
...
int dev_set_weight(char *dev, int size)
{
	int s;
	struct ifreq ifr;
