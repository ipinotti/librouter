...
int librouter_wifi_ssid_get (char * ssid_name, int ssid_size)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_SSID_KEY, ssid_name, ssid_size) < 0)
		return -1;
...
int librouter_wifi_ssid_broadcast_enable_set (int enable)
{
	...
	if (enable)
		return librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_SSID_BROADCAST_KEY, "0");
	else
		return librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_SSID_BROADCAST_KEY, "1");
...
int librouter_wifi_ssid_broadcast_enable_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_SSID_BROADCAST_KEY, enable, sizeof(enable)) < 0)
		return -1;
...
int librouter_wifi_channel_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_CHANNEL_KEY, ch, sizeof(ch)) < 0)
		return -1;
...
int librouter_wifi_hw_mode_set (wifi_hw_mode hw_mode)
{
	...
	switch(hw_mode){
		case a_hw:
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_KEY, "a") < 0)
				return -1;
			break;
		case b_hw:
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_KEY, "b") < 0)
				return -1;
			break;
		case g_hw:
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_KEY, "g") < 0)
				return -1;
			break;
		case n_hw:
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_KEY, "g") < 0)
				return -1;
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_N_KEY, "1") < 0)
				return -1;
			break;
		default:
			return -1;
...
int librouter_wifi_hw_mode_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_KEY, hw, sizeof(hw)) < 0)
		return -1;

	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_HW_MODE_N_KEY, hw_N, sizeof(hw_N)) < 0)
		return -1;
...
int librouter_wifi_max_num_sta_set (int max_num_sta)
{
	...
	if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_MAX_NUM_STA_KEY, sta_num) < 0)
		return -1;
...
int librouter_wifi_max_num_sta_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_MAX_NUM_STA_KEY, sta_num, sizeof(sta_num)) < 0)
			return -1;
...
int librouter_wifi_beacon_inteval_set (int beacon_inter)
{
	...
	if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_BEACON_INTRVAL_KEY, beacon_value) < 0)
		return -1;
...
int librouter_wifi_beacon_inteval_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_BEACON_INTRVAL_KEY, beacon_value, sizeof(beacon_value)) < 0)
			return -1;
...
int librouter_wifi_rts_threshold_set (int rts_thresh)
{
	...
	if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_RTS_THRESHOLD_KEY, rts_value) < 0)
		return -1;
...
int librouter_wifi_rts_threshold_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_RTS_THRESHOLD_KEY, rts_value, sizeof(rts_value)) < 0)
			return -1;
...
int librouter_wifi_fragm_threshold_set (int fragm_thresh)
{
	...
	if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_FRAGM_THRESHOLD_KEY, fragm_value) < 0)
		return -1;
...
int librouter_wifi_fragm_threshold_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_FRAGM_THRESHOLD_KEY, fragm_value, sizeof(fragm_value)) < 0)
		return -1;
...
int librouter_wifi_dtim_inter_set (int dtim_inter)
{
	...
	if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_DTIM_INTER_KEY, dtim_value) < 0)
		return -1;
...
int librouter_wifi_dtim_inter_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_DTIM_INTER_KEY, dtim_value, sizeof(dtim_value)) < 0)
			return -1;
...
int librouter_wifi_preamble_type_set (wifi_preamble_type preamble_T)
{
	switch(preamble_T){
		case long_p:
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_PREAMBLE_KEY, "0") < 0)
				return -1;
			break;
		case short_p:
			if (librouter_str_replace_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_PREAMBLE_KEY, "1") < 0)
				return -1;
			break;
		default:
			return -1;
...
int librouter_wifi_preamble_type_get (void)
{
	...
	if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_PREAMBLE_KEY, preamble, sizeof(preamble)) < 0)
		return -1;
...
int librouter_wifi_security_mode_get (librouter_wifi_security_mode_struct * security)
{
	...
	if (librouter_str_find_string_in_file_return_stat(WIFI_CONFIG_FILE, "auth_algs=1")){
		if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_WEP_PASS_KEY, security->wep_key, member_size(librouter_wifi_security_mode_struct, wep_key)) < 0)
			return -1;
		...
		if (librouter_str_find_string_in_file_return_stat(WIFI_CONFIG_FILE, "auth_algs=0")){
				security->security_mode = wep;
	...
	if (librouter_str_find_string_in_file_return_stat(WIFI_CONFIG_FILE, "wpa")){
		if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_SEC_WPA_MODE_KEY, mode, sizeof(mode)) < 0)
			return -1;
		switch (atoi(mode)) {
			case 1:
		...
		if (librouter_str_find_string_in_file_return_stat(WIFI_CONFIG_FILE, WIFI_WPA_PSK_KEY)){
			if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_WPA_PSK_KEY, security->wpa_psk, member_size(librouter_wifi_security_mode_struct, wpa_psk)) < 0)
				return -1;
		}
		else
			if (librouter_str_find_string_in_file_without_space(WIFI_CONFIG_FILE, WIFI_WPA_PHR_KEY, security->wpa_phrase, member_size(librouter_wifi_security_mode_struct, wpa_phrase)) < 0)
				return -1;
...
wpa_set:
			if (strlen(wifi_cfg.security.wpa_phrase) > 7){
