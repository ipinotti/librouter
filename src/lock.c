...
static int _semaphore_state(int sem_id, char action)
{
	struct sembuf sem_b;
...
static int _get_sem_id(unsigned int key, int *sem_id)
{
...
int librouter_lock_rmon_config_access(void)
{
	...
	if (_get_sem_id(RMON_SEM_KEY, &sem_id) == 0)
		return 0;

	for (count = 0; count < 1000; count++) {
		if (_semaphore_state(sem_id, 'p') == 1)
			return 1;
...
int librouter_unlock_rmon_config_access(void)
{
	...
	for (count = 0; count < 5000; count++) {
		if (_semaphore_state(sem_id, 'v') == 1)
			return 1;
...
int librouter_lock_snmp_tree_access(void)
{
	...
	/* Semaphore: make the lock */
	if (_get_sem_id(SNMP_TREE_SEM_KEY, &sem_id) == 0)
		return 0;
...
int librouter_unlock_snmp_tree_access(void)
{
	...
	syslog(LOG_ERR, "Not possible to unlock semaphore: %d",
	                SNMP_TREE_SEM_KEY);
	return 0;
}
...
int librouter_lock_del_mod_access(unsigned int wait_time)
{
	...
	/* Semaphore: make the lock */
	if (_get_sem_id(DELETE_MOD_SEM_KEY, &sem_id) == 0)
		return 0;

	if (wait_time > 0) {
		/* converte para o limite do loop */
		wait_time *= 1000;
