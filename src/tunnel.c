	...
	if (dev_exists(name)) {
		...
		if (conf_format) {
			...
			if (p.iph.saddr || p.iph.daddr) {
				...
				if (p.iph.daddr) {
					address.s_addr=p.iph.daddr;
					fprintf(out, "destination %s", inet_ntoa(address));
				}
				fprintf(out, "\n");
			}
			switch(p.iph.protocol) {
				case IPPROTO_IPIP:
					fprintf(out, "  Tunnel protocol/transport IP/IP\n");
					break;
				case IPPROTO_GRE:
				default:
					fprintf(out, "  Tunnel protocol/transport GRE/IP");
					...
					if (p.i_flags & GRE_CSUM) {
						fprintf(out, ", checksumming of packets enabled");
					} else {
						fprintf(out, ", checksumming of packets disabled");
					}
					fprintf(out, "\n");
					break;
			}
